...
class GlobalLocalBrainAge(nn.Module):
    def __init__(self,inplace,
        ...
        self.global_feat = i.InceptionV1()
        self.local_feat = i.InceptionV1()
        hidden_size = 512
    ...
    def forward(self,xinput):
        ...
        print(xglo.shape)
        #xglo = xglo.reshape(1,512,int(xglo.shape[0]),1)
        ...
        for y in range(0,H-self.patch_size+1,self.step):
            for x in range(0,W-self.patch_size+1,self.step):
                ...
                if xloc.shape[2]== 270:
                    xloc = xloc.reshape(30,512,9,1)
                elif xloc.shape[2]== 135:
                    xloc = xloc.reshape(15,512,9,1)
                else:
                    xloc=xloc.reshape(1,512,9,1)
                #print(xloc.shape[2])
