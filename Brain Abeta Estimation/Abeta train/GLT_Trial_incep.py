...
from GlobalLocalTransformer_incep import GlobalAttention,convBlock,GlobalLocalBrainAge
import pandas as pd
import math
import random
import json
...
root_directory="/Users/kathakolisengupta/Desktop/RA3/gray_map_5_incep/"
'''
manual_slice = {
...
def test_main(fold,test_loader_a,test_loader_s,test_loader_c,a_model_path,s_model_path,c_model_path,config=None,val=False):
    ...
    axial_model = GlobalLocalBrainAge(2,patch_size=64,step=32,nblock=6,backbone='InceptionV1')
    ...
    print("------------------------------------------------------------------------------")
    sagittal_model = GlobalLocalBrainAge(2,patch_size=64,step=32,nblock=6,backbone='InceptionV1')
    ...
    print("------------------------------------------------------------------------------")
    coronal_model = GlobalLocalBrainAge(2,patch_size=64,step=32,nblock=6,backbone='InceptionV1')
...
if __name__ == '__main__':
    ...
    for fold in [1,2,3]:
        ...
        model = GlobalLocalBrainAge(2,patch_size=config["patch_size"],step=32,nblock=6,backbone='InceptionV1')
        ...
        model = GlobalLocalBrainAge(2,patch_size=config["patch_size"],step=-1,nblock=6,backbone='InceptionV1')
        fill_locations(model,d_test)
        model = nn.DataParallel(model).to(device)
