...
class GlobalLocalBrainAge(nn.Module):
    ...
    def combine_output(self, global_output, local_output):
        # Here you can define how you want to combine global and local outputs
        # For example, you could simply add them, concatenate them, or apply any other strategy
        combined = global_output + local_output
        return combined
        
    def forward(self,xinput):
        _,_,self.H,self.W=xinput.size()
        outlist = []
        ...
        for y in range(0,self.H-self.patch_size,self.step):
            for x in range(0,self.W-self.patch_size,self.step):
                if not self.done:
        ...
        return outlist
        #aggregated_loc_output = sum(outlist) / len(outlist)

        # Combine global and aggregated local outputs
        #combined_output = self.combine_output(glo, aggregated_loc_output)
