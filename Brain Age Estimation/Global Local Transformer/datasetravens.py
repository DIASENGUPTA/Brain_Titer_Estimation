...
class BrainAgeDataset(Dataset):
    def __init__(self, data_dir,fold, train_test_val):
        ...
        self.data_list2_final = [self.data_list2[i] for i in shuffling_indices]
        ...
        if train_test_val=="train":
            ...
            self.data_list2_send=self.data_list2_final[int(self.ind(fold+1)[0]*0.15* len(self.data_list1_final)):int(self.ind(fold+1)[1]*0.15* len(self.data_list1_final))]\
                                +self.data_list2_final[int(0.15*self.ind(fold+2)[0] * len(self.data_list1_final)):int(0.15*self.ind(fold+2)[1]* len(self.data_list1_final))]\
                                +self.data_list2_final[int(0.15*self.ind(fold+3)[0]* len(self.data_list1_final)):int(0.15*self.ind(fold+3)[1]* len(self.data_list1_final))]\
                                +self.data_list2_final[int(0.15*self.ind(fold+4)[0] * len(self.data_list1_final)):int(0.15*self.ind(fold+4)[1]* len(self.data_list1_final))]
            ...
            self.data_list2_send=self.data_list2_final[int(self.ind(fold)[0]*0.15* len(self.data_list1_final)):int(self.ind(fold)[1]*0.15* len(self.data_list1_final))]
            ...
            self.data_list2_send=self.data_list2_final[int(0.75 * len(self.data_list1_final)):int(len(self.data_list1_final))]
    ...
    def __getitem__(self, idx):
        ...
        img2 = nib.load(self.data_list2_send[idx]).get_fdata()
        img2=img2[26:156,24:194,:]
        img_stack1 = np.empty((img1.shape[0], img1.shape[1], 0))
        img_stack2 = np.empty((img1.shape[0], img1.shape[1], 0))
        for i in range(60,120,12):
            #img = nib.Nifti1Image(img1[:,:,i], np.eye(4))
            #nib.save(img, 'slice'+str(i)+'.nii.gz')
            img_stack1=np.dstack((img_stack1,img1[:,:,i]))
            img_stack2=np.dstack((img_stack2,img2[:,:,i]))
       ...
       # age = torch.tensor(self.s,dtype=torch.float64)#convert actions to tensor
        ...
        return (img_stack1, img_stack2 , age)
