...
with open('input_list_NACC1.txt') as file:
    for line in file:
# Set the input folder name
        input_folder = line.strip()

# Construct the input and output file names
        path=f"/projectnb/bucbi/NACC_T1"
        subj=input_folder
        input_f_name = f"/projectnb/bucbi/NACC_T1/{input_folder}"
...
/projectnb/bucbi/dramms-1.5.1/bin/dramms -S {input_file} -T /share/pkg.7/fsl/6.0.4/install/data/standard/MNI152_T1_1mm_brain.nii.gz\
 ...
 -L {binary_mask} -l 1
        ...
        with open("script.sh", "w") as f:
                f.write(text)
        os.system(f"qsub -N {input_folder} -P bucbi script.sh")